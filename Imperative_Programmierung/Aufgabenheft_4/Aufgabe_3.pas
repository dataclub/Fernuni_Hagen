...
{ testet die Prozedur BerechneTiefeUndMaxTiefe }
  ...
  if inRefWurzel <> nil then
      begin
      { Knotentiefe ermitteln und zuweisen }
      inRefWurzel^.Tiefe := inTiefe;
      { ioMaxTiefe aktualisieren }
      if ioMaxTiefe < inTiefe then
        ioMaxTiefe := inTiefe;
      { Tiefe der Soehne ermitteln }
      BerechneTiefeUndMaxTiefe
      (inRefWurzel^.links,(inTiefe + 1), ioMaxTiefe);
      BerechneTiefeUndMaxTiefe
      (inRefWurzel^.rechts,(inTiefe + 1), ioMaxTiefe)
      end { if }
  end; { BerechneTiefeUndMaxTiefe }
